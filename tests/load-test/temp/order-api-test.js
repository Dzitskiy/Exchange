// Запуск теста по сценарию: k6 run order-api-test.js
...
const products = new SharedArray('orders', function () {
    return JSON.parse(open('./payload.json'));
});
...
export default function () {
    ...
    const response = http.post('http://localhost:5000/api/Orders', payload, params);
    const end = Date.now();
